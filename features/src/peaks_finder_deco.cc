                                       ...
                                       int close_peaks_radius_param)
{
    ...
    if (nfoundZ > 0)
    {
        ...
        for (size_t s = 0; s < num_peaks; s++)
        {
            ...
            TH2D *h2d_filtered_x =
                new TH2D("FILTER_X", "", size_cell[2], 0, size_cell[2], size_cell[0], 0, size_cell[0]);
            ...
            for (int i = 0; i < nbins; i++)
                source[i] = projX[s]->GetBinContent(i + 1);
            TSpectrum *spectrumX = new TSpectrum(20, res);
            Int_t nfoundX =
                spectrumX->SearchHighRes(source, dest, nbins, sigma, threshold, kFALSE, ndeconv, kFALSE, window);
            for (int i = 0; i < nbins; i++)
                projX[s]->SetBinContent(i + 1, dest[i]);
            ...
            while (!nfoundX)
            {
                ...
                {
                    for (int i = 0; i < nbins; i++)
                        projX[s]->SetBinContent(i + 1, source[i]);
            ...
            TH2D *h2d_filtered_y =
                new TH2D("FILTER_Y", "", size_cell[2], 0, size_cell[2], size_cell[1], 0, size_cell[1]);
            ...
            for (int i = 0; i < nbins; i++)
                source[i] = projY[s]->GetBinContent(i + 1);
            TSpectrum *spectrumY = new TSpectrum(20, res);
            int nfoundY =
                spectrumY->SearchHighRes(source, dest, nbins, sigma, threshold, kFALSE, ndeconv, kFALSE, window);
            for (int i = 0; i < nbins; i++)
                projY[s]->SetBinContent(i + 1, dest[i]);
            ...
            while (!nfoundY)
            {
                ...
                {
                    for (int i = 0; i < nbins; i++)
                        projY[s]->SetBinContent(i + 1, source[i]);
        ...
        while ((file = (TSystemFile *)next()))
        {
            ...
            if (!file->IsDirectory())
            {
                ...
                for (size_t i = 0; i < 1000; i++)
                {
                    ...
                    oFile << file->GetName() << "\t" << i << "\t" << info[13] << "\t" << info[0] << "\t" << info[3]
                          << "\t" << info[4] << "\t" << info[5] << "\t" << info[6] << "\t" << info[7] << "\t" << info[8]
                          << "\t" << info[9] << "\t" << info[10] << "\t" << info[11] << "\t" << info[12] << std::endl;
