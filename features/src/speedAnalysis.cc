...
// int close_start_radius_param = 5; // Maximum distance to consider a cell close to the first interaction (5 if 100 100
// 100 and 3 if 50 50 100)
...
std::vector<double> speedAnalysis(TString filePath, int eventNum, std::vector<int> size_cell, double threshold,
                                  int close_start_radius_param, int xyWindowSize_param, int zWindowSize_param)
{
    ...
    for (size_t j = 0; j < Tentries; j++)
    {
        ...
        if ((int_pos[0] >= minWin && int_pos[0] <= maxWin) && (int_pos[1] >= minWin && int_pos[1] <= maxWin))
        {
    ...
    for (size_t j = 0; j < Tentries; j++)
    {
        ...
        if (int_pos[2] == 0 && abs(int_pos[0] - ((size_cell[0] - 1) / 2)) < close_start_radius_param &&
            abs(int_pos[1] - ((size_cell[0] - 1) / 2)) < close_start_radius_param)
        {
...
void fillTable(std::string particleName, std::vector<int> size_cell, double threshold, int close_start_radius_param,
               int xyWindowSize_param, int zWindowSize_param)
{

    std::string outFile = "./results_" + std::to_string(size_cell[0]) + "_" + std::to_string(size_cell[1]) + "_" +
            ...
            if (!file->IsDirectory())
            {
                ...
                for (size_t i = 0; i < 1000; i++)
                {
                    ...
                    std::vector<double> info =
                        speedAnalysis(fileName, i, size_cell, threshold, close_start_radius_param, xyWindowSize_param,
                                      zWindowSize_param);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 9)
    {
        std::cerr << "Usage: " << argv[0]
                  << " <particle> <size_x> <size_y> <size_z> <threshold> <closeFirstInteraction> <xyWindow> <zWindow>"
                  << std::endl;
        return 1;
    }
